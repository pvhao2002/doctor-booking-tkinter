...
from utils.alert import show_warning_v2, show_info_v2
from utils.constant import LOGIN, REGISTER
...
class LoginPage:
    def __init__(self, window):
        ...
        self.window.title('App quản lý đặt lịch khám bệnh')
        self.current_user = None
        self.mode = LOGIN
        # ============================background image============================
        self.bg_frame = Image.open('images\\bg_resize.png')
        ...
        # ============ Left Side Image ================================================
        self.side_image = Image.open('images\\bg_main.png')
        photo = ImageTk.PhotoImage(self.side_image, size=(500, 500))
        ...
        # ============================username====================================
        self.username_label = Label(self.lgn_frame, text="Tên đăng nhập", bg="#040405", fg="#4f4e4d",
        ...
        # =========== Sign Up ==================================================
        self.sign_label = Label(self.lgn_frame, text='Chưa có tài khoản?', font=("yu gothic ui", 11, "bold"),
    ...
    def login(self):
        if self.mode == LOGIN:
            ...
            if result is not None:
                ...
                show_warning_v2("Tài khoản hoặc mật khẩu không đúng")
        else:
            username = self.username_entry.get()
            password = self.password_entry.get()
            result = self.service.register(username, password, username)
            if result is not None:
                show_info_v2("Đăng ký thành công")
                self.switch()
            else:
                show_warning_v2("Đăng ký không thành công")

    def switch(self):
        if self.mode == LOGIN:
            self.mode = REGISTER
            self.login.configure(text="Đăng ký")
            self.sign_in_label.configure(text="Đăng ký")
            self.sign_label.configure(text="Đã có tài khoản?")
            self.signup_button_label.configure(image=self.login_img)
        else:
            self.mode = LOGIN
            self.login.configure(text="Đăng nhập")
            self.sign_in_label.configure(text="Đăng nhập")
            self.sign_label.configure(text="Chưa có tài khoản?")
            self.signup_button_label.configure(image=self.signup_img)
